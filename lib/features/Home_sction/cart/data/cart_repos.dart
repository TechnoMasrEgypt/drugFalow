...
Future<ApiResult<CartResponseModel>> updateCartItem({required int id,
    required UpdateCartItemRequest body,
  }) async {
    try {
      final response =
          await _apiService.updateCartItem(id, body);
      return ApiResult.success(response);
    } catch (e) {
      return ApiResult.failure(ErrorHandler.handle(e));
    }
...
Future<ApiResult<CartResponseModel>> saveCartAsDraft(int id) async {
  try {
    final response = await _apiService.saveCartAsDraft(
      id,
      {"is_drafted": 1},
    );
